    ...
    */
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let taskDto = items[indexPath.row] // 해당 인덱스의 taskdto 가져오기
            let taskDtoId = taskDto.id
            
            // Delete the row from the data source
            items.remove(at: (indexPath as NSIndexPath).row) // 원본 데이터 지움
            tableView.deleteRows(at: [indexPath], with: .fade) // 화면 요소 지움
            
            if let taskId = taskDtoId { // taskDtoId 값이 db에 존재할 때만 실행
                manager.deleteData(id: taskId) // db 데이터 지움
            }
